CREATE PROCEDURE s_checkUsername(IN username VARCHAR(100))
BEGIN
    IF (SELECT user_name FROM utilisateurs) = username
        PRINT 1
    ELSE
        PRINT  0
...
s_checkUsername(crillow);
alter PROCEDURE s_checkUsername(IN username VARCHAR(100))
BEGIN
    IF (SELECT username FROM utilisateurs)
        THEN PRINT 1;
    ELSE
        PRINT  0;
...
alter PROCEDURE s_checkUsername(IN username VARCHAR(100))
BEGIN
    IF EXISTS (SELECT username FROM utilisateurs)
        RETURN PRINT 1;
    ELSE
        RETURN   0;
END ;
...
BEGIN
    ...
    ELSE
        RETURN   0;
END ;
create function fun_checkUsername(IN username VARCHAR(100))
RETURNS TINYINT 
BEGIN
    IF EXISTS (SELECT user_name FROM utilisateurs WHERE user_name LIKE username)
        RETURN 1;
...
ENDIF 
END;
alter function s_checkUsername(IN username VARCHAR(100))
BEGIN
    IF EXISTS (SELECT user_name FROM utilisateurs WHERE user_name LIKE username)
        THEN RETURN 1;
    ELSE
...
ENDIF 
END;
create PROCEDURE  p_checkUsername(IN username VARCHAR(100))
BEGIN
    IF EXISTS (SELECT user_name FROM utilisateurs WHERE user_name LIKE username)
        THEN RETURN 1;
    ELSE
...
ENDIF 
END;
call p_checkUsername('crillow');
call p_checkUsername('haha');
alter PROCEDURE  p_checkUsername(IN username VARCHAR(100))
BEGIN
    IF EXISTS (SELECT user_name FROM utilisateurs WHERE user_name LIKE username)
        THEN 
        print 1;
...
END;
call p_checkUsername('haha');
select fun_checkUsername('haha');
select fun_checkUsername('crillow');
call p_checkUsername('haha');
